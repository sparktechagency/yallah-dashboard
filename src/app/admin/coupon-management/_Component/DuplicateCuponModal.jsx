...
export default function DuplicateCuponeModal({ open, setOpen, couponId }) {
  ...
  const handleFormSubmit = async ({ couponType, ...value }) => {
    ...
    const payload = {
      ...
      terms: termsData,
      arabicHowToUse: howToUseDataAr,
      arabicTerms: termsDataAr,
    ...
    try {
      const response = await addCupon(payload).unwrap();
      if (response?.success) {
        toast.success(response?.message || "Coupon duplicated successfully");
        setOpen(false);
      }
    } catch (error) {
